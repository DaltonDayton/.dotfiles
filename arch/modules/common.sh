...
function validate_installation() {
  ...
  "command")
    if command -v "$component" &>/dev/null; then
      ...
      log_error "$component is not available in PATH"
      return 1
    fi
    ;;
  "symlink")
    if [ -L "$target" ] && [ -e "$target" ]; then
      log_success "Symlink $target is correctly set"
      return 0
    else
      log_error "Symlink $target is missing or broken"
      return 1
    fi
    ;;
  "file")
    if [ -f "$target" ]; then
      log_success "File $target exists"
      return 0
    else
      log_error "File $target is missing"
      return 1
    fi
    ;;
  *)
